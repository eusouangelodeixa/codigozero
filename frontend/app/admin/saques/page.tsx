...
export default function AdminWithdrawalsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/admin/${basePath}${params}`, { headers: hdr() });
  ...
  const act = async (id: string, action: "approve" | "reject") => {
    ...
    setBusyId(id);
    try {
      const res = await fetch(`${API}/api/admin/${basePath}/${id}/${action}`, {
        method: "POST",
        headers: hdr(),
  ...
  return (
    <div className={styles.page}>
      <div className={styles.pageHeader}>
        <h1 className={styles.pageTitle}>Saques</h1>
        <p className={styles.pageDesc}>
          Aprove saques após pagar via M-Pesa/eMola, ou rejeite para devolver o saldo.
        </p>
      </div>
        ...
        <div className={styles.filterRow}>
          {[
            ...
            <button
              key={s.key}
              type="button"
              className={`${styles.filterBtn} ${source === s.key ? styles.filterBtnActive : ""}`}
        ...
        {loading ? (
          ...
          <table className={styles.table}>
            <thead>
              <tr>
                <th>Solicitado em</th>
                <th>{source === "afiliados" ? "Afiliado" : "Sócio"}</th>
                <th>Bruto</th>
                <th>Taxa</th>
                <th>Líquido</th>
                <th>Método</th>
                  ...
                  <tr key={w.id}>
                    ...
                    <td>
                      <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                        <strong>{party?.displayName || party?.user?.name}</strong>
